      ...
      const { date,id } = req.body;
       ...
       await db.query("UPDATE PrescribedTests SET Scheduled_Date = ? WHERE PrescribedTests.PT_id = ?",[date, id],  (err, result)=>{
          ...
          else 
          {
            res.status(200).send(result);
            console.log(result);
            db.query("SELECT D.Name aP.Name as P_Name,DATE(Scheduled_date) as Date from PrescribedTests as PT,Patient as P,Doctor as D,Test as T where PT.PT_id= ? AND PT.Patient_id=P.Patient_id AND PT.Doctor_id=D.Doctor_id AND T.Test_id=PT.Test_id",[id],(err1,result1)=>{
               if(!err1)
               {
